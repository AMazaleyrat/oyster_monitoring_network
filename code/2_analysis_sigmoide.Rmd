...
except Marseillan, located in the Mediterranean Thau lagoon, for which tidal variations are only tenuous and Men-er-Roué
...
```{r echo=FALSE}
# growth_death %>%
#   group_by(campaign, site, class_age) %>%
#   filter(DOY == min(DOY)) %>%
#   ungroup() %>%
#   summarize(median(DOY)) %>%
#   as.numeric()
#
# growth_death %>%
#   group_by(campaign, site, class_age) %>%
...
make prediction between DOY 65 (median day of seeding date) and DOY 337 (median day of the end of the monitoring).
...
(mass on DOY 337 divided by mass on DOY 65)
